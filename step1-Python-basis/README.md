...
1. 默认JetBrains，点击Install开始安装。安装完成后点击Finish结束（不要勾选Run PyCharm）
...
1. 点击确定
1. 勾选License server，在License server address：处填入`http：//idea.lanyus.com`,点击确定
1. 点击确定
1. 安装完成
...
+ [4.1](#4.1) **Python注释**
  ...
  注释的目的是让人能轻松阅读每一行代码的意义，同时也为程序员后期代码维护提供了方便。在Python中，一共有两种代码注释，一种是[单行注释](#4.1)，另一种是[多行注释](#4.1)
...
+ [4.2](#4.2) **Python 的行与缩进**
  ...
  *错误示例：*
  ...
  if True:
     ...
     print('Yes')
  ```
  *注意，下面这样的代码由于在不同代码块，所以**不会报错** ,但是我们绝不不提倡*
  ```python
  if True:
      print('True')
  else:
